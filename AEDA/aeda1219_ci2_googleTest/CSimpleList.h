	...
	void intercalar(const CSimpleList &lst) //Grupo 2 c)
	{
	    ...
	    while (true){
	        if (segundo == 0) break;
	            ...
	            insert_end(segundo->data());
	            segundo = segundo->next();
	        }
	        else{
	            CNode* aux = primeiro->next();
	            CNode* aux2 = segundo->next();
	            CNode* aux3 = new CNode(segundo->data(), segundo->next());
	            primeiro->setNext(aux3);
	            aux3->setNext(aux);
	            primeiro = aux;
	            segundo = aux2;
	        }
