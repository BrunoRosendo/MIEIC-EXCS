...
public class BucketState extends State {
    private final Bucket b1;
    private final Bucket b2;

    public BucketState(int start_b1, int start_b2, int max_b1, int max_b2) {
        this(
    ...
    private BucketState(final Bucket b1, final Bucket b2, final BucketState parent) {
        super(parent);
        this.b1 = b1;
        this.b2 = b2;
    }
    ...
    public boolean test(final State target) {
        if (!(target instanceof BucketState))
            return false;

        final BucketState bucketTarget = (BucketState) target;
        return b1.getQuantity() == bucketTarget.b1.getQuantity();
    }
    ...
    private BucketState transfer(final Bucket b1, final Bucket b2, boolean revert) {
        ...
        return revert ? new BucketState(newB2, newB1, this) : new BucketState(newB1, newB2, this);
    }

    private BucketState fillB1() {
        return new BucketState(b1.filled(), b2, this);
    }

    private BucketState fillB2() {
        return new BucketState(b1, b2.filled(), this);
    }

    private BucketState emptyB1() {
        return new BucketState(b1.empty(), b2, this);
    }

    private BucketState emptyB2() {
        return new BucketState(b1, b2.empty(), this);
    }
